...
#		temp_mu_folders=filter(os.path.isdir, os.listdir('.'))
	temp_mu_folders=os.getcwd()	
	folder_item=temp_mu_folders

# --------- check for sim.h5 files

	
	print "folder item is "+ folder_item
	os.chdir(start_directory + "/vertex_run")
	...
	print("I am in " + os.getcwd())
	print("Directory is "+ start_directory + "/vertex_run" +"/run_DF/sigma_output_nodal.dat")
	print("Should I be running in here? "+ str(os.path.exists(start_directory +"/vertex_run" +"/run_DF/sigma_output_nodal.dat")))

	DF_running_path=os.getcwd()+"/runningDF.dat"
	
	if os.path.exists(DF_running_path)==False:
		os.system("echo 'blah'> runningDF.dat")
		if os.path.exists(start_directory + "/vertex_run" +"/run_DF/sigma_output_nodal.dat") == False or run_anyways:
			...
			if os.path.exists(os.getcwd()+"/vert_F_phpp")==False or rerun_inverter==True:
				os.system("sh ../../../../prog_runs_scripts/run_inverter_inplace.sh")
		...
		#	print("I am in " + os.getcwd())	
			...
			if os.path.exists(os.getcwd()+"/output.h5")==False or rerun_DF==True:
				...
				os.system("$HOME/alps_core/opendf/install/bin/hub_df_cubic2d --input qmc_output.h5 --nbosonic 15 --df_sc_cutoff 1.0e-6 --df_sc_iter 50 --df_sc_mix 0.4 --fluct_diag 1 --mu "+str(mu))
				

			os.system("python ../../../../../prog_runs_scripts/extract_DF.py")

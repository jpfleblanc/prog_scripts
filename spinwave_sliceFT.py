...
for i in range(0, len(qx)):
	...
	if abs(qy[i]-0) <0.00001 and qx[i]<=numpy.pi:
		length=qx[i] + numpy.pi + 2*numpy.pi/math.sqrt(2) + numpy.pi/2.0
		slice5.append((qx[i],qy[i],wp[i],length))


numpy.savetxt('sliceFT1.dat',slice1)
numpy.savetxt('sliceFT2.dat',slice2)
numpy.savetxt('sliceFT3.dat',slice3)
numpy.savetxt('sliceFT4.dat',slice4)
numpy.savetxt('sliceFT5.dat',slice5)

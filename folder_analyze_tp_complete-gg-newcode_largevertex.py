...
# --------- check for sim.h5 files
		for folder_item in temp_mu_folders:
			...
			print("I am in " + os.getcwd())
			print("Directory is "+ start_directory + "/"+ item+"/"+folder_item+"/vertex_run" +"/run_DF_gg/sigma_output_nodal.dat")
			print("Should I be running in here? "+ str(os.path.exists(start_directory + "/"+ item+"/"+folder_item+"/vertex_run" +"/run_DF_gg/sigma_output_nodal.dat")))

			DF_running_path=os.getcwd()+"/runningDF.dat"
			
			if os.path.exists(DF_running_path)==False:
				os.system("echo 'blah'> runningDF.dat")
				if os.path.exists(start_directory + "/"+ item+"/"+folder_item+"/vertex_run" +"/run_DF_gg/sigma_output_nodal.dat") == False or run_anyways:
					...
					if os.path.exists(os.getcwd()+"/run_DF_gg")==False:
						os.system("mkdir run_DF_gg")
					os.chdir(os.getcwd()+"/run_DF_gg")	
				#	print("I am in " + os.getcwd())	
					...
					if os.path.exists(os.getcwd()+"/output.h5")==False or rerun_DF==True:
						...
						run_string="$HOME/alps_core/opendf/install/bin/hub_df_square_nnn --input qmc_output.h5  --df_sc_cutoff 1.0e-8 --df_sc_iter 1200 --df_sc_mix 0.2 --fluct_diag 0 --nbosonic 32 --add_lattice_bubble 1 --mu "+str(mu) +" --tp "+str(tprime) +" --resume 1"
						print run_string

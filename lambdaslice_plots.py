...
qx, qy, lam= numpy.loadtxt(inputfile, unpack=True)
...
for i in range(0, len(qx)):
	...
	if qx[i]>=numpy.pi and qx[i]<=numpy.pi*3/2 and abs(qy[i]-(qx[i]-numpy.pi))<0.0001:
		...
		slice3.append((qx[i],qy[i],lam[i],length))
	if qx[i]<=numpy.pi/2 and qy[i]==qx[i]:
		length=numpy.pi/2-qx[i] + numpy.pi + numpy.pi*math.sqrt(2)
		slice4.append((qx[i],qy[i],lam[i],length))
	if qy[i]==0 and qx[i]<=numpy.pi:
		length=qx[i] + numpy.pi + 2*numpy.pi/math.sqrt(2) + numpy.pi/2.0
		slice5.append((qx[i],qy[i],lam[i],length))


numpy.savetxt('lamslice1.dat',slice1)
numpy.savetxt('lamslice2.dat',slice2)
numpy.savetxt('lamslice3.dat',slice3)
numpy.savetxt('lamslice4.dat',slice4)
numpy.savetxt('lamslice5.dat',slice5)

...
for s in range(0,len(qlist)):
	...
	for i in range(0,len(data_array)):
		data_array[i]=((data_array[i][0],data_array[i][1]/norm,data_array[i][2],data_array[i][3]/norm,data_array[i][4]))

	#for dtau in numpy.linspace(-10,10,1000):	
	...
	data_FT.append((qlist[s][0],qlist[s][1], dtau, numpy.real(ft_result), numpy.imag(ft_result)))
...
numpy.savetxt('FT_betao2.dat',data_FT)

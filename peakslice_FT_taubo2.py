...
qx, qy, wp, chi, chi_im= numpy.loadtxt(inputfile, unpack=True)
...
for i in range(0, len(qx)):
	...
	if qx[i]>= numpy.pi/2 and qx[i] <= numpy.pi and qx[i]==qy[i]:
		...
		slice1.append((qx[i],qy[i],wp[i],chi[i],chi_im[i],length))
	if abs(qx[i]-numpy.pi)<0.0001 and qy[i] <= numpy.pi:
		length=(numpy.pi-qy[i]) + numpy.pi/math.sqrt(2)
		slice2.append((qx[i],qy[i],wp[i],chi[i],chi_im[i],length))
	if qx[i]>=numpy.pi and qx[i]<=numpy.pi*3/2 and abs(qy[i]-(qx[i]-numpy.pi))<0.0001:
		...
		slice3.append((qx[i],qy[i],wp[i],chi[i],chi_im[i],length))
	if qx[i]<=numpy.pi/2 and qy[i]==qx[i]:
		length=numpy.pi/2-qx[i] + numpy.pi + numpy.pi*math.sqrt(2)
		slice4.append((qx[i],qy[i],wp[i],chi[i],chi_im[i],length))
	if qy[i]==0 and qx[i]<=numpy.pi:
		length=qx[i] + numpy.pi + 2*numpy.pi/math.sqrt(2) + numpy.pi/2.0
		slice5.append((qx[i],qy[i],wp[i],chi[i],chi_im[i],length))


numpy.savetxt('sliceFT_tau1.dat',slice1)
numpy.savetxt('sliceFT_tau2.dat',slice2)
numpy.savetxt('sliceFT_tau3.dat',slice3)
numpy.savetxt('sliceFT_tau4.dat',slice4)
numpy.savetxt('sliceFT_tau5.dat',slice5)

...
def eval():
	...
	for dir_items in dirs:
		if "Betts2D" in dir_items:
			...
			if "-ref" not in dir_items:# and "_" not in dir_items:
...
#--------- read in relevant data from sim.h5
				...
				if True:#f["/parameters/MU"].value<=0:
					...
					if "/simulation/results/density_up_times_sign/mean/value" in f:
						...
						sz_mean=(f["/simulation/results/Sz_times_sign/mean/value"].value)
						sz_mean=sum(sz_mean)/float(len(sz_mean))/Sign
						sz_error=(f["/simulation/results/Sz_times_sign/mean/error"].value)
						sz_error=sum(sz_error)/float(len(sz_error))/Sign
						...
						density_mean_err = sum(density_mean_err)/float(len(density_mean_err))
...
#sum(i*i for i in density_mean_err)
						...
						sz_mean=(f["/simulation/results/Sz/mean/value"].value)
						sz_mean=sum(sz_mean)/float(len(sz_mean))
						sz_error=(f["/simulation/results/Sz/mean/error"].value)
						sz_error=sum(sz_error)/float(len(sz_error))
					...
					print "Found t' =" +str(tprime)	

					if True:#site >150 and site < 32:
						print "Evaluating sisj"
						sisjnn, sisjnnerror, nndistance= nnsisj_eval(os.getcwd()+"/"+folder_item+"/sim.h5")
					else:
						sisjnn=float('NaN')
...
#							read_file.close()
							...
							try:
								...
								energy_optionstr=' --siteration '+str(Iteration-(iterationstart-1))+' --niteration '+str(Iteration-1)+' --directory '+os.getcwd()+ '/'+folder_item+' --nfreq '+str(nfreq)+' --nsite '+str(site)+' --mu '+str(mu)+' --beta '+ str(beta)+' --U '+ str(Uvalue) +' '	
								try:
									os.system("/global/homes/l/leblanc/Hopper/alps_svn/Energy/energy "+ 	energy_optionstr+">> energy.dat")
								except:
									pass		
...
#---------- all data in following list	
					if(len(fileslist)>iterationstart):		
						fig1b.append( ( Uvalue, T,  mu, ekin_mean+epot_mean, site,  doubleoc_mean, doubleoc_error, ekin_mean, ekin_error, epot_mean, epot_error, density_mean, density_mean_err, sz_mean, sz_error, sisjnn, sisjnnerror, Sign, tprime))
...
#create complete database
	...
	database_file=open('database-mar27_U4_filled_cleanup_2015-simons.dat', 'w+')
	for item in fig1b:
		database_file.write(str(item[0])+" "+str(item[1])+" "+str(item[2])+" "+ str(item[3])+" "+str(item[4])+" "+str(item[5])+" "+str(item[6])+" "+str(item[7])+" "+str(item[8])+" "+str(item[9])+" "+str(item[10])+" "+str(item[11])+" "+str(item[12])+" "+str(item[13])+" "+str(item[14])+" "+str(item[15])+" "+str(item[16])+" "+str(item[17])+" "+str(item[18])+"\n")
